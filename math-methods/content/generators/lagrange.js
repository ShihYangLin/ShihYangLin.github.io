...
const kinds=[{value:'max',label:both('Local maximum','局部極大值')},{value:'min',label:both('Local minimum','局部極小值')}];
...
function setup(rng,level){
  ...
  return {id:'lagrange/setup',level,vars:['x','y','lambda'],domain:{x:[1,5],y:[1,5],lambda:[0.5,3]},
    ...
    hints:[both('The objective is the first term.','標的函數是第一項。'),both('Subtract the entire left side of the constraint from its right side inside the multiplier term.','在乘數項中，用限制式右側減去整個左側。')],
...
function solve(rng,level){
  ...
  return {id:'lagrange/solve-linear',level,vars:[],domain:{},
    ...
    hints:[both('Write the three first-order equations, including the constraint.','寫出三個第一階條件，包括限制式。'),both('Set $-2(x-a)=\\lambda$ and $-2(y-b)=\\lambda$, then use $x+y=c$.','令 $-2(x-a)=\\lambda$、$-2(y-b)=\\lambda$，再使用 $x+y=c$。')],
    solution:[both(`$Z=-(x-${a})^2-(y-${b})^2+\\lambda(${c}-x-y)$. The FOCs give $-2(x-${a})=\\lambda=-2(y-${b})$ and $x+y=${c}$. Hence $(x^*,y^*,\\lambda^*)=(${x},${y},${lambda})$.`,`$Z=-(x-${a})^2-(y-${b})^2+\\lambda(${c}-x-y)$。第一階條件給出 $-2(x-${a})=\\lambda=-2(y-${b})$ 與 $x+y=${c}$，故 $(x^*,y^*,\\lambda^*)=(${x},${y},${lambda})$。`),both(`With the border $(g_x,g_y)=(1,1)$, $\\bar H=\\begin{pmatrix}0&1&1\\\\1&-2&0\\\\1&0&-2\\end{pmatrix}$ and $|\\bar H|=${det}>0$: a strict constrained local maximum. The quadratic is strictly concave, so it is also globally best on the feasible line.`,`以 $(g_x,g_y)=(1,1)$ 作邊，$\\bar H=\\begin{pmatrix}0&1&1\\\\1&-2&0\\\\1&0&-2\\end{pmatrix}$，$|\\bar H|=${det}>0$，故為嚴格的受限局部極大值；標的函數嚴格凹，也是在可行線上的全域極大值。`)]};
}
function cobb(rng,level){
  ...
  return {id:'lagrange/cobb-douglas',level,vars:[],domain:{},
    ...
    solution:[both(`$Z=${a===1?'x':`x^{${a}}`}${b===1?'y':`y^{${b}}`}+\\lambda(${B}-${px===1?'':px}x-${py===1?'':py}y)$. The FOCs $U_x/${px}=U_y/${py}=\\lambda$ imply $x=y$; the budget gives $x^*=y^*=${k}$ and $\\lambda^*=${frac(k**(a+b-1),priceScale,true)}>0$.`,`$Z=${a===1?'x':`x^{${a}}`}${b===1?'y':`y^{${b}}`}+\\lambda(${B}-${px===1?'':px}x-${py===1?'':py}y)$。第一階條件 $U_x/${px}=U_y/${py}=\\lambda$ 得 $x=y$；預算式給出 $x^*=y^*=${k}$、$\\lambda^*=${frac(k**(a+b-1),priceScale,true)}>0$。`),both(`At the solution, $(U_{xx},U_{xy},U_{yy})=(${hxx},${hxy},${hyy})$ and $(g_x,g_y)=(${px},${py})$. Thus $|\\bar H|=${det}>0$, a strict local maximum; Cobb–Douglas level curves and the positive budget line make this the unique interior global maximum.`,`在解上，$(U_{xx},U_{xy},U_{yy})=(${hxx},${hxy},${hyy})$、$(g_x,g_y)=(${px},${py})$，故 $|\\bar H|=${det}>0$，為嚴格局部極大值；Cobb–Douglas 等效用曲線與正價格預算線也給出唯一的內部全域極大值。`)]};
}
function cost(rng,level){
  ...
  return {id:'lagrange/cost-min',level,vars:[],domain:{},
    ...
    misconceptions:[error('det',-det,'For one equality and two choices, a negative bordered determinant supports a constrained minimum.','單一等式限制、兩個抉擇變數下，負的加邊行列式支持受限局部極小值。')],
    ...
    solution:[both(`$Z=${w}L+${r}K+\\lambda(${Q}-${production})$. At the solution $(g_L,g_K)=(${gx},${gy})$, so $w=\\lambda g_L$, $r=\\lambda g_K$, and $g=${Q}$ give $(L^*,K^*,\\lambda^*)=(${L},${K},${lambda})$.`,`$Z=${w}L+${r}K+\\lambda(${Q}-${production})$。解上 $(g_L,g_K)=(${gx},${gy})$，故 $w=\\lambda g_L$、$r=\\lambda g_K$ 與 $g=${Q}$ 給出 $(L^*,K^*,\\lambda^*)=(${L},${K},${lambda})$。`),both(`The border is $(g_L,g_K)=(${gx},${gy})$ and $(Z_{LL},Z_{LK},Z_{KK})=(${hxx},${hxy},0)$. Thus $|\\bar H|=${det}<0$: a strict constrained local minimum. Substitution $K=Q_0/(${denominator})$ gives a strictly convex cost curve for $L>0$, so it is global.`,`邊為 $(g_L,g_K)=(${gx},${gy})$，且 $(Z_{LL},Z_{LK},Z_{KK})=(${hxx},${hxy},0)$，故 $|\\bar H|=${det}<0$，為嚴格的受限局部極小值。代入 $K=Q_0/(${denominator})$ 後，$L>0$ 的成本曲線嚴格凸，故也是全域極小值。`)]};
}
function borderedProblem(rng,level){
  ...
  return {id:'lagrange/bordered-hessian',level,vars:[],domain:{},
    prompt:both(`For $f(x,y)=${objective}$ under $g(x,y)=${p===1?'':p}x+${q===1?'':q}y=${c}$, the constrained stationary point is $(${x},${y})$. ${convention.en} Compute $|\\bar H|$ with border $(g_x,g_y)$ and classify it on the feasible line.${level===3?' Also find the multiplier.':''}`,`在 $g(x,y)=${p===1?'':p}x+${q===1?'':q}y=${c}$ 下，$f(x,y)=${objective}$ 的受限平穩點是 $(${x},${y})$。${convention.zh}以 $(g_x,g_y)$ 作邊求 $|\\bar H|$，並在可行線上判別。${level===3?'另求乘數。':''}`),
    fields:[field('det','number','$|\\bar H|$ =','$|\\bar H|$ =',det),field('kind','choice','Constrained classification =','受限判別 =',kind,kinds),...(level===3?[field('lambda','number','$\\lambda^*$ =','$\\lambda^*$ =',lambda)]:[])],
    misconceptions:[error('kind',kind==='max'?'min':'max','The bordered determinant sign rule is positive for a constrained maximum and negative for a minimum.','加邊行列式的符號規則：受限極大為正，受限極小為負。')],
    hints:[both('Place zero at the top-left and the constraint gradient along the top and left.','左上角放零，限制式梯度放在上邊與左邊。'),both('For two choices and one equality, positive determinant means local maximum; negative means local minimum.','兩變數、單一等式限制下，行列式正為局部極大，負為局部極小。')],
    solution:[both(`At $(${x},${y})$, $(f_x,f_y)=(${lambda*p},${lambda*q})=${lambda}(g_x,g_y)$, so $\\lambda^*=${lambda}\\ne0$. The bordered matrix is $\\bar H=\\begin{pmatrix}0&${p}&${q}\\\\${p}&${hxx}&${cross}\\\\${q}&${cross}&${hyy}\\end{pmatrix}$, giving $|\\bar H|=${det}$.`,`在 $(${x},${y})$，$(f_x,f_y)=(${lambda*p},${lambda*q})=${lambda}(g_x,g_y)$，故 $\\lambda^*=${lambda}\\ne0$。加邊矩陣為 $\\bar H=\\begin{pmatrix}0&${p}&${q}\\\\${p}&${hxx}&${cross}\\\\${q}&${cross}&${hyy}\\end{pmatrix}$，得 $|\\bar H|=${det}$。`),both(`The sign is ${kind==='max'?'positive':'negative'}, giving a strict constrained local ${kind==='max'?'maximum':'minimum'}. This sign concerns curvature along feasible directions.`,`符號為${kind==='max'?'正':'負'}，故是嚴格的受限局部${kind==='max'?'極大':'極小'}值；此符號檢驗可行方向上的曲度。`)]};
}
