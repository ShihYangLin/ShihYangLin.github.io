...
function cobb(rng,level){
  const a=rng.pick([1,2]),b=1,k=rng.int(2,5),px=a,py=b,B=(a+b)*k,lambda=k**(a+b-1);
  ...
  return {id:'lagrange/cobb-douglas',level,vars:[],domain:{},
    ...
    solution:[both(`$Z=${a===1?'x':`x^{${a}}`}${b===1?'y':`y^{${b}}`}+\\lambda(${B}-${px===1?'':px}x-${py===1?'':py}y)$. The FOCs $U_x/${px}=U_y/${py}=\\lambda$ imply $x=y$; the budget gives $x^*=y^*=${k}$ and $\\lambda^*=${lambda}>0$.`,`$Z=${a===1?'x':`x^{${a}}`}${b===1?'y':`y^{${b}}`}+\\lambda(${B}-${px===1?'':px}x-${py===1?'':py}y)$。一階條件 $U_x/${px}=U_y/${py}=\\lambda$ 得 $x=y$；預算式給出 $x^*=y^*=${k}$、$\\lambda^*=${lambda}>0$。`),both(`At the solution, $(U_{xx},U_{xy},U_{yy})=(${hxx},${hxy},${hyy})$ and $(g_x,g_y)=(${px},${py})$. Thus $|\\bar H|=${det}>0$, a strict local maximum; Cobb–Douglas level curves and the positive budget line make this the unique interior global maximum.`,`在解上，$(U_{xx},U_{xy},U_{yy})=(${hxx},${hxy},${hyy})$、$(g_x,g_y)=(${px},${py})$，故 $|\\bar H|=${det}>0$，為嚴格相對極大值；Cobb–Douglas 等效用曲線與正價格預算線也給出唯一的內部全域極大值。`)]};
}
function cost(rng,level){
  const L=rng.int(2,5),K=rng.int(2,5),w=K,r=L,Q=L*K,lambda=1,det=bordered(K,L,0,-lambda,0,'min');
  return {id:'lagrange/cost-min',level,vars:[],domain:{},
    prompt:both(`A firm must produce $Q_0=${Q}$ with $g(L,K)=LK$. Input prices are $w=${w}$ for labor and $r=${r}$ for capital; minimize $C=wL+rK$ for $L,K>0$. ${convention.en} Find the least-cost inputs, $\\lambda^*$, and $|\\bar H|$.`,`廠商必須以 $g(L,K)=LK$ 生產 $Q_0=${Q}$。勞動價格 $w=${w}$、資本價格 $r=${r}$；在 $L,K>0$ 下極小化 $C=wL+rK$。${convention.zh}求最低成本投入、$\\lambda^*$ 與 $|\\bar H|$。`),
    ...
    hints:[both('At the tangency, each input has the same price per marginal product.','切點上，兩種投入的每單位邊際產量價格相同。'),both('Use $w/K=r/L=\\lambda$ and $LK=Q_0$.','聯立 $w/K=r/L=\\lambda$ 與 $LK=Q_0$。')],
    solution:[both(`$Z=${w}L+${r}K+\\lambda(${Q}-LK)$. FOCs give $w=\\lambda K$, $r=\\lambda L$, and $LK=${Q}$. Hence $(L^*,K^*,\\lambda^*)=(${L},${K},1)$, with positive inputs and multiplier.`,`$Z=${w}L+${r}K+\\lambda(${Q}-LK)$。一階條件是 $w=\\lambda K$、$r=\\lambda L$、$LK=${Q}$，得 $(L^*,K^*,\\lambda^*)=(${L},${K},1)$；投入及乘數皆為正。`),both(`The border is $(g_L,g_K)=(${K},${L})$ and the Lagrangian Hessian has $Z_{LL}=Z_{KK}=0$, $Z_{LK}=-1$. Thus $|\\bar H|=${det}<0$: a strict constrained local minimum. Substitution $K=Q_0/L$ gives a strictly convex cost curve for $L>0$, so it is global.`,`邊為 $(g_L,g_K)=(${K},${L})$，Lagrange 函數 Hessian 有 $Z_{LL}=Z_{KK}=0$、$Z_{LK}=-1$，故 $|\\bar H|=${det}<0$，為嚴格限制式相對極小值。代入 $K=Q_0/L$ 後，$L>0$ 的成本曲線嚴格凸，故也是全域極小值。`)]};
}
function borderedProblem(rng,level){
  const kind=rng.pick(['max','min']),s=kind==='max'?-1:1,a=rng.int(1,3),b=rng.int(1,3),p=rng.int(1,2),q=rng.int(1,2),x=rng.int(1,4),y=rng.int(1,4),c=p*x+q*y;
  const hxx=2*s*a,hyy=2*s*b,det=bordered(p,q,hxx,0,hyy,kind);
  return {id:'lagrange/bordered-hessian',level,vars:[],domain:{},
    prompt:both(`For $f(x,y)=${s<0?'-':''}${a===1?'':a}(x-${x})^2${s<0?'-':'+'}${b===1?'':b}(y-${y})^2$ under $g(x,y)=${p===1?'':p}x+${q===1?'':q}y=${c}$, the stationary point is $(${x},${y})$. ${convention.en} Compute $|\\bar H|$ with border $(g_x,g_y)$ and classify it on the feasible line.`,`在 $g(x,y)=${p===1?'':p}x+${q===1?'':q}y=${c}$ 下，$f(x,y)=${s<0?'-':''}${a===1?'':a}(x-${x})^2${s<0?'-':'+'}${b===1?'':b}(y-${y})^2$ 的駐點是 $(${x},${y})$。${convention.zh}以 $(g_x,g_y)$ 作邊求 $|\\bar H|$，並在可行線上判別。`),
    fields:[field('det','number','$|\\bar H|$ =','$|\\bar H|$ =',det),field('kind','choice','Constrained classification =','限制式判別 =',kind,kinds)],
    ...
    solution:[both(`$\\bar H=\\begin{pmatrix}0&${p}&${q}\\\\${p}&${hxx}&0\\\\${q}&0&${hyy}\\end{pmatrix}$, so $|\\bar H|=${det}$.`,`$\\bar H=\\begin{pmatrix}0&${p}&${q}\\\\${p}&${hxx}&0\\\\${q}&0&${hyy}\\end{pmatrix}$，故 $|\\bar H|=${det}$。`),both(`The sign is ${kind==='max'?'positive':'negative'}, giving a strict constrained local ${kind==='max'?'maximum':'minimum'}. This sign concerns curvature along feasible directions.`,`符號為${kind==='max'?'正':'負'}，故是嚴格限制式相對${kind==='max'?'極大':'極小'}值；此符號檢驗可行方向上的曲率。`)]};
}
function shadow(rng,level){
  const k=rng.int(2,6),B=2*k,delta=rng.int(1,3),lambda=k,approx=k*delta,exact=approx+delta*delta/4;
  bordered(1,1,0,1,0,'max');
  return {id:'lagrange/shadow-price',level,vars:[],domain:{},
    prompt:both(`Maximize $U(x,y)=xy$ with $x+y=B=${B}$ and $x,y>0$. ${convention.en} At the optimum $\\lambda^*=${lambda}$. If budget rises by $\\Delta B=${delta}$, use $\\Delta U^*\\approx\\lambda^*\\Delta B$ as a first-order approximation, then give the exact change after re-solving.`,`在 $x+y=B=${B}$、$x,y>0$ 下極大化 $U(x,y)=xy$。${convention.zh}最適時 $\\lambda^*=${lambda}$。若預算增加 $\\Delta B=${delta}$，先以 $\\Delta U^*\\approx\\lambda^*\\Delta B$ 作一階近似，再重新求解給出精確變化量。`),
    ...
    hints:[both('The multiplier is the slope of optimized utility with respect to budget at the original budget.','乘數是原預算處最適效用對預算的斜率。'),both('For any positive budget $B$, re-solve $x^*=y^*=B/2$ and $U^*(B)=B^2/4$.','對任意正預算 $B$，重新求解得 $x^*=y^*=B/2$、$U^*(B)=B^2/4$。')],
    solution:[both(`The original optimum is $(x^*,y^*)=(${k},${k})$ and $U^*=${k*k}$. First-order approximation: $\\lambda^*\\Delta B=${lambda}(${delta})=${approx}$.`,`原最適解為 $(x^*,y^*)=(${k},${k})$、$U^*=${k*k}$。一階近似為 $\\lambda^*\\Delta B=${lambda}(${delta})=${approx}$。`),both(`Exactly, $U^*(B)=B^2/4$; at $B'=${B+delta}$ the re-solved choices are $x'=y'=${frac(B+delta,2,true)}$. Thus the exact change is $U^*(${B+delta})-U^*(${B})=${frac(4*approx+delta*delta,4,true)}=${frac(4*exact,4,true)}$. The difference from the first-order estimate is $${frac(delta*delta,4,true)}$.`,`精確地，$U^*(B)=B^2/4$；在 $B'=${B+delta}$，重新求解得 $x'=y'=${frac(B+delta,2,true)}$。因此精確變化是 $U^*(${B+delta})-U^*(${B})=${frac(4*exact,4,true)}$，與一階近似差 $${frac(delta*delta,4,true)}$。`)]};
}
export const lagrangeGenerators=[
  {id:'setup',title:both('Set up the Lagrangian','建立 Lagrange 函數'),levels:[1],generate:setup},
  {id:'solve-linear',title:both('Quadratic objective, linear constraint','二次目標與線性限制'),levels:[1,2],generate:solve},
  {id:'cobb-douglas',title:both('Cobb–Douglas consumer','Cobb–Douglas 消費者'),levels:[2],generate:cobb},
  {id:'cost-min',title:both('Least-cost inputs','最低成本投入組合'),levels:[2,3],generate:cost},
  {id:'bordered-hessian',title:both('Bordered Hessian test','加邊 Hessian 檢定'),levels:[2,3],generate:borderedProblem},
  {id:'shadow-price',title:both('Shadow value and exact change','影子價格與精確變化'),levels:[3],generate:shadow}
];
