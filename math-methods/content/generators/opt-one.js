...
function criticalPoints(rng, level) {
  ...
  return {
    ...
    prompt: both(`For the smooth function $f(x)=${f}$ on $\\mathbb R$, find all stationary $x$ values, where $f^{(1)}(x)=0$. Enter a set.`, `對定義於 $\\mathbb R$ 的平滑函數 $f(x)=${f}$，求所有滿足 $f^{(1)}(x)=0$ 的駐點 $x$ 值，並以集合輸入。`),
    ...
    solution: [both(`Differentiation gives $f^{(1)}(x)=${fp}$.`, `微分得 $f^{(1)}(x)=${fp}$。`), both(`This factors as $f^{(1)}(x)=${fp.startsWith('-') ? '-' : ''}6(${linear(1, 'x', -r1)})(${linear(1, 'x', -r2)})$. Hence the stationary $x$ values are $\\{${r1},${r2}\\}$.`, `因式分解得 $f^{(1)}(x)=${fp.startsWith('-') ? '-' : ''}6(${linear(1, 'x', -r1)})(${linear(1, 'x', -r2)})$，所以駐點 $x$ 值為 $\\{${r1},${r2}\\}$。`)]
  };
}
...
function classify(rng, level) {
  ...
  return {
    ...
    misconceptions: [error('left', rightKind, 'Check the sign of $f^{(2)}$ at the left root; the two roots have opposite curvature.', '請檢查左側駐點的二階導數符號；兩個駐點的曲率相反。')],
    hints: [both('First solve $f^{(1)}(x)=0$ for both candidates.', '先解 $f^{(1)}(x)=0$，找出兩個候選點。'), both('A negative $f^{(2)}$ at a stationary point gives a local maximum; a positive value gives a local minimum.', '駐點處二階導數為負是相對極大值，為正是相對極小值。')],
    solution: [both(`$f^{(1)}(x)=${fp}= ${sign > 0 ? '' : '-'}6(${linear(1, 'x', -r1)})(${linear(1, 'x', -r2)})$, so the stationary values are $\\{${r1},${r2}\\}$.`, `$f^{(1)}(x)=${fp}= ${sign > 0 ? '' : '-'}6(${linear(1, 'x', -r1)})(${linear(1, 'x', -r2)})$，故駐點值為 $\\{${r1},${r2}\\}$。`), both(`$f^{(2)}(x)=${fpp}$, giving $f^{(2)}(${r1})=${left}$ and $f^{(2)}(${r2})=${right}$. Therefore $x=${r1}$ is a local ${leftKind === 'max' ? 'maximum' : 'minimum'} and $x=${r2}$ a local ${rightKind === 'max' ? 'maximum' : 'minimum'}.`, `$f^{(2)}(x)=${fpp}$，得 $f^{(2)}(${r1})=${left}$、$f^{(2)}(${r2})=${right}$。因此 $x=${r1}$ 為相對${leftKind === 'max' ? '極大' : '極小'}值，$x=${r2}$ 為相對${rightKind === 'max' ? '極大' : '極小'}值。`)]
  };
}
...
function profitMax(rng, level) {
  ...
  return {
    ...
    prompt: both(`A firm faces inverse demand $P(Q)=${price}$ and cost $C(Q)=${cost}$. On feasible output $0<Q<${frac(a,b,true)}$ (where price is positive), find the profit-maximizing interior $Q^*>0$ and the sign of $\\pi^{(2)}(Q^*)$.${level === 3 ? ' Also find maximum profit.' : ''}`, `廠商的反需求為 $P(Q)=${price}$，成本為 $C(Q)=${cost}$。在價格為正的可行產量 $0<Q<${frac(a,b,true)}$ 上，求利潤極大的內部產量 $Q^*>0$，以及 $\\pi^{(2)}(Q^*)$ 的符號。${level === 3 ? '另求最大利潤。' : ''}`),
    fields: [field('q', 'number', '$Q^*$ =', '$Q^*$ =', q), field('soc', 'choice', 'Sign of $\\pi^{(2)}(Q^*)$ =', '$\\pi^{(2)}(Q^*)$ 的符號 =', 'negative', [{ value: 'negative', label: both('Negative', '負') }, { value: 'positive', label: both('Positive', '正') }]), ...(level === 3 ? [field('profit', 'number', '$\\pi(Q^*)$ =', '$\\pi(Q^*)$ =', profit)] : [])],
    misconceptions: [error('q', `-${q}`, 'Output must be positive and satisfy the feasible price range.', '產量必須為正，且在價格非負的可行範圍內。')],
    hints: [both('Write profit as revenue minus cost and differentiate.', '將利潤寫為收益減成本，再求導。'), both('At an interior solution set $MR=MC$; use $\\pi^{(2)}=R^{(2)}-C^{(2)}$ for the SOC.', '內部解令 $MR=MC$；二階條件使用 $\\pi^{(2)}=R^{(2)}-C^{(2)}$。')],
    solution: [both(`$R(Q)=Q(${price})$, so $MR=${mr}$ and $MC=${mc}$. The first-order condition $MR=MC$ gives $Q^*=${q}$.`, `$R(Q)=Q(${price})$，故 $MR=${mr}$、$MC=${mc}$。一階條件 $MR=MC$ 得 $Q^*=${q}$。`), both(`$\\pi^{(2)}(Q)=${soc}<0$ throughout the feasible interval, so $Q^*=${q}$ is a local maximum. Strict concavity makes it the unique global maximum on that interval; $P(${q})=${a - b * q}>0$.${level === 3 ? ` Profit is $\\pi(${q})=${profit}$.` : ''}`, `$\\pi^{(2)}(Q)=${soc}<0$ 在可行區間內處處成立，故 $Q^*=${q}$ 為局部極大值。嚴格凹性也保證它是該區間唯一的全域極大值；$P(${q})=${a - b * q}>0$。${level === 3 ? `利潤為 $\\pi(${q})=${profit}$。` : ''}`)]
  };
}
...
function inflection(rng, level) {
  ...
  return {
    ...
    misconceptions: [error('x', a + 1, 'A zero of $f^{(2)}$ is at the center, not one unit to its right; verify signs on both sides.', '二階導數為零的中心點才是候選點；還要檢查其兩側符號。')],
    hints: [both('Differentiate twice and solve $f^{(2)}(x)=0$.', '求二階導數，解 $f^{(2)}(x)=0$。'), both('Check the signs of $f^{(2)}$ just left and right of that point.', '檢查該點左右兩側的二階導數符號。')],
    solution: [both(`$f^{(2)}(x)=${fpp}$ vanishes at $x=${a}$.`, `$f^{(2)}(x)=${fpp}$ 在 $x=${a}$ 為零。`), both(`At $x=${a - 1}$, $f^{(2)}=${-6 * s}$; at $x=${a + 1}$, $f^{(2)}=${6 * s}$. The signs are opposite, so concavity truly changes ${s > 0 ? 'from concave to convex' : 'from convex to concave'} at $x=${a}$.`, `在 $x=${a - 1}$ 時 $f^{(2)}=${-6 * s}$；在 $x=${a + 1}$ 時 $f^{(2)}=${6 * s}$。符號相反，故 $x=${a}$ 確實${s > 0 ? '由凹轉凸' : '由凸轉凹'}，是反曲點。`)]
  };
}
