...
function wineStorage(rng,level){
 const u=rng.int(2,9),A=rng.int(2,12)*100,r=level===2?10:5,k=2*u/r,t=u*u;
 return {id:'timing/wine-storage',level,vars:[],domain:{},
 plot:{expr:`${A}*exp(${k}*sqrt(t)-t/${r})`,variable:'t',range:[0,2*t],axis:'A(t)',points:[{x:t,kind:'optimum'}]},
 ...
 misconceptions:[error('t',u,'The first-order condition determines $\\sqrt t$; square it to obtain years.','第一階條件先求出 $\\sqrt t$；求年數還須平方。')],
...
function timber(rng,level){
 ...
 plot:{expr:`${A}*${base}^(sqrt(t))*exp(-log(${base})*t/${2*u})`,variable:'t',range:[0,2*t],axis:'A(t)',points:[{x:t,kind:'optimum'}]},
 prompt:both(`Already planted timber has harvest value $V(t)=${texProduct(A, `${base}^{\\sqrt t}`)}$ for $t\\ge0$. Planting cost is sunk and upkeep is zero. At continuous discount rate $r=\\ln(${base})/${2*u}$, find the harvest time maximizing present value $A(t)=V(t)e^{-rt}$ and classify the interior stationary point.`,`已種植的林木於 $t\\ge0$ 時採伐，價值為 $V(t)=${texProduct(A, `${base}^{\\sqrt t}`)}$。種植成本為沉沒成本且不計維護成本。連續折現率 $r=\\ln(${base})/${2*u}$。求使現值 $A(t)=V(t)e^{-rt}$ 極大的採伐時點，並判別內部平穩點。`),
...
{id:'wine-storage',title:both('Wine storage','葡萄酒儲存時點'),levels:[2,3],minDistinct: 40, generate:wineStorage},
{id:'timber',title:both('Timber harvest','林木採伐時點'),levels:[3],minDistinct: 40, generate:timber}
];
