...
function wineStorage(rng,level){
 ...
 prompt:both(`A dealer already owns wine worth $V(t)=${A}e^{(${k})\\sqrt t}$ after $t\\ge0$ years. With zero storage cost and continuous discount rate $r=1/${r}$, choose the sale time maximizing $A(t)=V(t)e^{-rt}$.${level===3?' Also give the sign of the second derivative of $\\ln A$ at the interior solution.':''}`,`酒商已持有葡萄酒，$t\\ge0$ 年後的市值為 $V(t)=${A}e^{(${k})\\sqrt t}$。不計保管成本，連續折現率 $r=1/${r}$。求使現值 $A(t)=V(t)e^{-rt}$ 極大的出售時點。${level===3?'另判斷內部解處 $\\ln A$ 二階導數的符號。':''}`),
...
function timber(rng,level){
 ...
 prompt:both(`Already planted timber has harvest value $V(t)=${A}${base}^{\\sqrt t}$ for $t\\ge0$. Planting cost is sunk and upkeep is zero. At continuous discount rate $r=\\ln(${base})/${2*u}$, find the harvest time maximizing present value $A(t)=V(t)e^{-rt}$ and classify the interior stationary point.`,`已種植的林木於 $t\\ge0$ 時採伐，價值為 $V(t)=${A}${base}^{\\sqrt t}$。種植成本為沉沒成本且不計維護費。連續折現率 $r=\\ln(${base})/${2*u}$。求使現值 $A(t)=V(t)e^{-rt}$ 極大的採伐時點，並判別內部駐點。`),
