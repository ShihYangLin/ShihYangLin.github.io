import { polynomial, term } from './format.js';
...
function marginalProducts(rng, level) {
  const a = rng.int(1, 4), u = rng.int(1, 3), v = rng.int(1, 3), A = 2*a, K = u*u, L = v*v;
  ...
  return { id: 'partials/marginal-products', level, vars: ['K','L'], domain: { K: [0.5, 10], L: [0.5, 10] },
    ...
    solution: [both(`$MP_K=${A}(1/2)K^{-1/2}L^{1/2}=${a}\\sqrt{L}/\\sqrt{K}$.`, `$MP_K=${A}(1/2)K^{-1/2}L^{1/2}=${a}\\sqrt{L}/\\sqrt{K}$。`), both(`Similarly $MP_L=${a}\\sqrt{K}/\\sqrt{L}$. At the given inputs, $MP_K=${a}(${v})/${u}=${a*v}/${u}$.`, `同理，$MP_L=${a}\\sqrt{K}/\\sqrt{L}$。代入指定投入，$MP_K=${a}(${v})/${u}=${a*v}/${u}$。`)] };
}
...
function marketCs(rng, level) {
  ...
  return { id: 'partials/market-cs', level, vars: [], domain: {},
    prompt: both(`A market has $Q_d=${a}-${term(b,'P')}$ and $Q_s=-${c}+${term(d,'P')}$. Solve the equilibrium and find $\\partial P^*/\\partial a$, holding other parameters fixed. What is the sign of $\\partial Q^*/\\partial c$?${level === 3 ? ' Also give $\\partial Q^*/\\partial a$.' : ''}`, `市場需求為 $Q_d=${a}-${term(b,'P')}$，供給為 $Q_s=-${c}+${term(d,'P')}$。求均衡與固定其他參數時的 $\\partial P^*/\\partial a$，並判斷 $\\partial Q^*/\\partial c$ 的符號。${level === 3 ? '另求 $\\partial Q^*/\\partial a$。' : ''}`),
    fields: [field('p','number','Equilibrium price =','均衡價格 =',p),field('q','number','Equilibrium quantity =','均衡數量 =',q),field('pa','number','dP*/da =','dP*/da =',`1/${sum}`),field('qc','choice','Sign of dQ*/dc =','dQ*/dc 的符號 =','negative',signs),...(level===3?[field('qa','number','dQ*/da =','dQ*/da =',`${d}/${sum}`)]:[])],
    ...
    solution: [both(`Equating schedules gives $P^*=(a+c)/(b+d)=(${a}+${c})/${sum}=${p}$ and $Q^*=(ad-bc)/(b+d)=${q}$.`, `供需相等得 $P^*=(a+c)/(b+d)=(${a}+${c})/${sum}=${p}$，$Q^*=(ad-bc)/(b+d)=${q}$。`),both(`Therefore $\\partial P^*/\\partial a=1/${sum}$ and $\\partial Q^*/\\partial c=-${b}/${sum}<0$.${level===3?` Also $\\partial Q^*/\\partial a=${d}/${sum}$.`:''}`, `因此 $\\partial P^*/\\partial a=1/${sum}$，$\\partial Q^*/\\partial c=-${b}/${sum}<0$。${level===3?`另外 $\\partial Q^*/\\partial a=${d}/${sum}$。`:''}`)] };
}

function nationalIncomeCs(rng, level) {
  const m = rng.int(1, 3), tax = rng.int(0, 2), den = 16-4*m+m*tax;
  return { id:'partials/national-income-cs',level,vars:[],domain:{},
    prompt: both(`In $Y=C+I+G$, let $C=C_0+(${m}/4)(Y-T)$ and $T=T_0+(${tax}/4)Y$. Autonomous terms are fixed. Find the equilibrium spending multiplier $\\partial Y^*/\\partial G$${level===3?' and the tax-intercept effect $\\partial Y^*/\\partial T_0$':''}.`, `在 $Y=C+I+G$ 中，令 $C=C_0+(${m}/4)(Y-T)$、$T=T_0+(${tax}/4)Y$。固定其他自主項，求均衡支出乘數 $\\partial Y^*/\\partial G$${level===3?'，以及稅收截距效果 $\\partial Y^*/\\partial T_0$':''}。`),
    fields:[field('g','number','dY*/dG =','dY*/dG =',`16/${den}`),...(level===3?[field('t','number','dY*/dT_0 =','dY*/dT_0 =',`-${4*m}/${den}`)]:[])],
    ...
    solution:[both(`Substitution gives $[1-(${m}/4)(1-${tax}/4)]Y=C_0-(${m}/4)T_0+I+G$.`, `代入後得 $[1-(${m}/4)(1-${tax}/4)]Y=C_0-(${m}/4)T_0+I+G$。`),both(`The coefficient on $Y$ is $${den}/16$; hence $\\partial Y^*/\\partial G=16/${den}$.${level===3?` Likewise $\\partial Y^*/\\partial T_0=-${4*m}/${den}$.`:''}`, `$Y$ 的係數是 $${den}/16$，所以 $\\partial Y^*/\\partial G=16/${den}$。${level===3?`同理，$\\partial Y^*/\\partial T_0=-${4*m}/${den}$。`:''}`)] };
}
...
export const partialsGenerators=[
 {id:'partial-basic',title:both('Partial derivatives','偏導數'),levels:[1,2],generate:partialBasic},
 {id:'marginal-products',title:both('Marginal products','邊際產量'),levels:[2],generate:marginalProducts},
 {id:'market-cs',title:both('Market comparative statics','市場比較靜態分析'),levels:[2,3],generate:marketCs},
 {id:'national-income-cs',title:both('Income multiplier','所得乘數'),levels:[2,3],generate:nationalIncomeCs},
 {id:'jacobian',title:both('Jacobian determinant','Jacobian 行列式'),levels:[3],generate:jacobian}
];
