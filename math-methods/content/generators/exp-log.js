import { linear, term } from './format.js';
...
function logRules(rng,level){
  ...
  return {id:'exp-log/log-rules',level,vars:['x'],domain:{x:[0.5,4]},
    ...
    solution:[both(`$\\ln(x^{${n}})=${n}\\ln x$ because $x>0$.`,`因為 $x>0$，$\\ln(x^{${n}})=${n}\\ln x$。`),both(`Thus $${n}\\ln x-\\ln x=${n-1}\\ln x$.`,`因此 $${n}\\ln x-\\ln x=${n-1}\\ln x$。`)]};
}
...
function compound(rng,level){
 ...
 const annual=`${rate}/10`, exponent=m*n, discrete=`${principal}*(1+${rate}/${10*m})^${exponent}`;
 ...
 solution:[both(`There are $${m*n}$ periods, each at rate $${rate}/${10*m}$; hence $B_d=${principal}(1+${rate}/${10*m})^{${exponent}}$.`,`共有 $${m*n}$ 期，每期利率為 $${rate}/${10*m}$；故 $B_d=${principal}(1+${rate}/${10*m})^{${exponent}}$。`),both(level===2?`Numerically $B_d\\approx${(principal*(1+rate/(10*m))**exponent).toFixed(3)}$. Continuous compounding instead gives $B_c=${principal}e^{${rate*n}/10}\\approx${(principal*Math.exp(rate*n/10)).toFixed(3)}$.`:`Evaluating the power gives $B_d\\approx${(principal*(1+rate/(10*m))**exponent).toFixed(3)}$; the exact answer is $${principal}(1+${rate}/${10*m})^{${exponent}}$.`,level===2?`數值上 $B_d\\approx${(principal*(1+rate/(10*m))**exponent).toFixed(3)}$；連續複利則為 $B_c=${principal}e^{${rate*n}/10}\\approx${(principal*Math.exp(rate*n/10)).toFixed(3)}$。`:`計算冪次得 $B_d\\approx${(principal*(1+rate/(10*m))**exponent).toFixed(3)}$；精確答案為 $${principal}(1+${rate}/${10*m})^{${exponent}}$。`)]};
}
function growthRate(rng,level){
 ...
 prompt:both(`For $t>0$, let $Y(t)=${A}${k===1?'t':`t^{${k}}`}e^{${term(g,'t')}/10}$. Find its instantaneous proportional growth rate $d\\ln Y/dt$.`,`對 $t>0$，設 $Y(t)=${A}${k===1?'t':`t^{${k}}`}e^{${term(g,'t')}/10}$。求瞬時比例成長率 $d\\ln Y/dt$。`),
 fields:[field('rate','expr','Growth rate =','成長率 =',`${k}/t+${g}/10`)],misconceptions:[error('rate',`${g}/10`,'The power of time also contributes $k/t$ to proportional growth.','時間冪次也對比例成長率貢獻 $k/t$。')],
 ...
 solution:[both(`$\\ln Y=\\ln ${A}+${k}\\ln t+${term(g,'t')}/10$.`,`$\\ln Y=\\ln ${A}+${k}\\ln t+${term(g,'t')}/10$。`),both(`Differentiate: $d\\ln Y/dt=${k}/t+${g}/10$.`,`微分得 $d\\ln Y/dt=${k}/t+${g}/10$。`)]};
 return {id:'exp-log/growth-rate',level,vars:[],domain:{},
 prompt:both(`Output and labor grow as $Y=${A}e^{${term(g,'t')}/10}$ and $L=${B}e^{${term(h,'t')}/10}$. Find the instantaneous growth rate of output per worker $Y/L$.`,`產量與勞動分別為 $Y=${A}e^{${term(g,'t')}/10}$、$L=${B}e^{${term(h,'t')}/10}$。求每位勞工產量 $Y/L$ 的瞬時成長率。`),
 fields:[field('rate','number','Per-worker growth rate =','每位勞工產量成長率 =',`${g-h}/10`)],misconceptions:[error('rate',`${g+h}/10`,'A quotient has growth rate equal to numerator growth minus denominator growth.','商的成長率為分子成長率減去分母成長率。')],
 hints:[both('Write $\\ln(Y/L)=\\ln Y-\\ln L$.','寫成 $\\ln(Y/L)=\\ln Y-\\ln L$。'),both('Subtract labor growth from output growth.','以產量成長率減去勞動成長率。')],
 solution:[both(`$d\\ln Y/dt=${g}/10$ and $d\\ln L/dt=${h}/10$.`,`$d\\ln Y/dt=${g}/10$，$d\\ln L/dt=${h}/10$。`),both(`Therefore $d\\ln(Y/L)/dt=(${g}-${h})/10=${g-h}/10$.`,`因此 $d\\ln(Y/L)/dt=(${g}-${h})/10=${g-h}/10$。`)]};
}
