...
function diffExp(rng,level){
 ...
 misconceptions:[error('ans',wrong,natural?'Multiply by the derivative of the exponent.':'A general base contributes a factor $\\ln b$.',natural?'須乘上指數部分（內層函數）的導數。':'一般底數微分還要乘上 $\\ln b$。')],
 hints:[both('Name the exponent $u(x)$ before using the chain rule.','先將指數記為 $u(x)$，再用連鎖法則。'),both(natural?'Use $(e^u)\\prime=e^u u\\prime$.':'Use $(b^u)\\prime=b^u(\\ln b)u\\prime$.',natural?'使用 $(e^u)\\prime=e^u u\\prime$。':'使用 $(b^u)\\prime=b^u(\\ln b)u\\prime$。')],
 solution:[both(`The inner function is $u=${inside}$, so $u'=${a}$.`,`內層函數為 $u=${inside}$，因此 $u'=${a}$。`),both(natural?`Therefore $f'(x)=${a}e^{${inside}}$.`:`Therefore $f'(x)=${a}(\\ln ${base})${base}^{${inside}}$.`,natural?`所以 $f'(x)=${a}e^{${inside}}$。`:`所以 $f'(x)=${a}(\\ln ${base})${base}^{${inside}}$。`)]};
}
...
function elasticity(rng,level){
 ...
 return {id:'exp-log/elasticity',level,vars:[],domain:{},prompt:both(`For $x>0$, $y=${A}x^{-${n}}$. Find the signed point elasticity $d\\ln y/d\\ln x$.`,`對 $x>0$，$y=${A}x^{-${n}}$。求含正負號的點彈性 $d\\ln y/d\\ln x$。`),fields:[field('eps','number','Signed elasticity =','彈性（含正負號）=',-n)],misconceptions:[error('eps',n,'The signed elasticity is negative for this decreasing function.','函數遞減，彈性應為負值。')],hints:[both('Take the log of both sides.','兩邊取對數。'),both('Differentiate $\\ln y=\\ln A-n\\ln x$ with respect to $\\ln x$.','將 $\\ln y=\\ln A-n\\ln x$ 對 $\\ln x$ 微分。')],solution:[both(`$\\ln y=\\ln ${A}-${texProduct(n, '\\ln x')}$.`,`$\\ln y=\\ln ${A}-${texProduct(n, '\\ln x')}$。`),both(`Thus $d\\ln y/d\\ln x=-${n}$ at every positive $x$.`,`故對所有正的 $x$，$d\\ln y/d\\ln x=-${n}$。`)]};}
 return {id:'exp-log/elasticity',level,vars:[],domain:{},prompt:both(`Demand is $Q(P)=${a}-${term(b,'P')}$. At $P=${p}$, where $Q=${q}>0$, find the signed price elasticity $d\\ln Q/d\\ln P$.`,`需求為 $Q(P)=${a}-${term(b,'P')}$。在 $P=${p}$ 且 $Q=${q}>0$ 時，求含正負號的價格彈性 $d\\ln Q/d\\ln P$。`),fields:[field('eps','number','Signed elasticity =','彈性（含正負號）=',frac(-b*p,q))],misconceptions:[error('eps',frac(b*p,q),'The demand slope is negative; retain its sign before taking any absolute value.','需求斜率為負；除非另要求絕對值，否則應保留負號。')],hints:[both('Point elasticity is slope times input divided by output.','點彈性等於斜率乘自變數再除以應變數。'),both('Use $(dQ/dP)(P/Q)$ at the stated point.','在指定點計算 $(dQ/dP)(P/Q)$。')],solution:[both(`$dQ/dP=-${b}$ and $Q(${p})=${q}$.`,`$dQ/dP=-${b}$，且 $Q(${p})=${q}$。`),both(`Hence $d\\ln Q/d\\ln P=(-${b})(${frac(p,q,true)})=${frac(-b*p,q,true)}$.`,`所以 $d\\ln Q/d\\ln P=(-${b})(${frac(p,q,true)})=${frac(-b*p,q,true)}$。`)]};
}
