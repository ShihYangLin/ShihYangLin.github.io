...
const kinds = ['max', 'min', 'saddle', 'inconclusive'].map((value, i) => ({ value, label: both(['Local maximum', 'Local minimum', 'Saddle', 'Hessian test inconclusive'][i], ['局部極大值', '局部極小值', '鞍點', 'Hessian 檢定無法判定'][i]) }));
...
function foc(rng, level) {
  ...
  return { id: 'opt-multi/foc-2var', level, vars: [], domain: {},
    prompt: both(`Find the stationary point of $f(x,y)=${q.expression}$.${level === 2 ? ' Classify it using the Hessian.' : ''}`, `求 $f(x,y)=${q.expression}$ 的平穩點。${level === 2 ? '再用 Hessian 矩陣判別。' : ''}`),
    ...
    hints: [both('Set both partial derivatives to zero.', '將兩個偏導數同時設為零。'), both('The squared terms reveal the center; check both leading principal minors.', '平方項顯示中心；再檢查兩個領導主要子行列式。')],
    solution: [both(`At $(${q.x},${q.y})$, both partial derivatives vanish. The linear first-order system has this unique solution.`, `在 $(${q.x},${q.y})$，兩個偏導數皆為零；此線性第一階條件組有唯一解。`), both(`$H=\\begin{pmatrix}${q.hxx}&${q.hxy}\\\\${q.hxy}&${q.hyy}\\end{pmatrix}$ has $D_1=${q.hxx}$ and $D_2=${q.D}>0$. It is ${q.hxx < 0 ? 'negative' : 'positive'} definite, so the point is a strict local ${q.hxx < 0 ? 'maximum' : 'minimum'}.`, `$H=\\begin{pmatrix}${q.hxx}&${q.hxy}\\\\${q.hxy}&${q.hyy}\\end{pmatrix}$ 的 $D_1=${q.hxx}$、$D_2=${q.D}>0$，故為${q.hxx < 0 ? '負' : '正'}定，平穩點是嚴格局部${q.hxx < 0 ? '極大' : '極小'}值。`)] };
}
function hessian(rng, level) {
  ...
  return { id: 'opt-multi/hessian', level, vars: [], domain: {},
    ...
    hints: [both('Differentiate each first partial once more.', '將各一階偏導數再微分一次。'), both('For a symmetric 2 by 2 Hessian, $|H|=f_{xx}f_{yy}-f_{xy}^2$.', '對稱的二階 Hessian 矩陣有 $|H|=f_{xx}f_{yy}-f_{xy}^2$。')],
    solution: [both(`The entries are $f_{xx}=${q.hxx}$, $f_{xy}=f_{yx}=${q.hxy}$, and $f_{yy}=${q.hyy}$.`, `各元素為 $f_{xx}=${q.hxx}$、$f_{xy}=f_{yx}=${q.hxy}$、$f_{yy}=${q.hyy}$。`), both(`Hence $|H|=(${q.hxx})(${q.hyy})-(${q.hxy})^2=${q.D}$. The first leading minor is $D_1=${q.hxx}$.`, `所以 $|H|=(${q.hxx})(${q.hyy})-(${q.hxy})^2=${q.D}$；第一個領導主要子行列式是 $D_1=${q.hxx}$。`)] };
}
function classify(rng, level) {
  ...
  return { id:'opt-multi/classify-2var',level,vars:[],domain:{},
    prompt:both(`At its stationary point, what conclusion does the Hessian test give for $f(x,y)=${f}$?`, `在平穩點處，Hessian 檢定對 $f(x,y)=${f}$ 得出什麼結論？`),
    fields:[field('kind','choice','Hessian conclusion =','Hessian 檢定結論 =',kind,kinds)],
    ...
    hints:[both('Compute both leading principal minors.','計算兩個領導主要子行列式。'),both('Positive determinant plus a negative first minor gives a maximum; negative determinant gives a saddle.','行列式正且第一領導主要子行列式負為極大；行列式負則為鞍點。')],
    solution:[both(`$D_1=${hxx}$ and $D_2=${D}$.`, `$D_1=${hxx}$、$D_2=${D}$。`),both(`The Hessian test is ${kind==='inconclusive'?'inconclusive when both minors vanish':kind==='saddle'?'indefinite, so the stationary point is a saddle':kind==='max'?'negative definite, giving a strict local maximum':'positive definite, giving a strict local minimum'}.`, `Hessian 矩陣${kind==='inconclusive'?'兩個領導主要子行列式皆為零，檢定無法判定':kind==='saddle'?'不定，因此平穩點是鞍點':kind==='max'?'負定，因此為嚴格局部極大值':'正定，因此為嚴格局部極小值'}。`)] };
}
function firm(rng, level) {
  ...
  return {id:'opt-multi/multiproduct-firm',level,vars:[],domain:{},
    ...
    misconceptions:[error('det',-D,'Profit curvature must be checked with the determinant of its Hessian.','請用利潤 Hessian 行列式檢查曲度。')],
    ...
    solution:[both(`FOCs are $${p1}=${2*a}Q_1+${b===1?'':b}Q_2+${u}$ and $${p2}=${b===1?'':b}Q_1+${2*d}Q_2+${v}$. They give $(Q_1^*,Q_2^*)=(${x},${y})$.`, `第一階條件為 $${p1}=${2*a}Q_1+${b===1?'':b}Q_2+${u}$ 與 $${p2}=${b===1?'':b}Q_1+${2*d}Q_2+${v}$，解得 $(Q_1^*,Q_2^*)=(${x},${y})$。`),both(`$H_\\pi=\\begin{pmatrix}${-2*a}&${-b}\\\\${-b}&${-2*d}\\end{pmatrix}$; its leading minors are $${-2*a}<0$ and $${D}>0$. Strict concavity gives the unique global profit maximum over nonnegative outputs.`, `$H_\\pi=\\begin{pmatrix}${-2*a}&${-b}\\\\${-b}&${-2*d}\\end{pmatrix}$；領導主要子行列式為 $${-2*a}<0$、$${D}>0$，嚴格凹性保證非負產量上的唯一全域利潤極大值。`)]};
}
function discrimination(rng, level) {
  ...
  return {id:'opt-multi/price-discrimination',level,vars:[],domain:{},
    ...
    hints:[both('Each market has its own marginal revenue; total output determines common cost.','各市場有自己的邊際收益；總產量決定共同成本。'),both('Set $MR_1=MR_2=MC$ and verify a negative definite profit Hessian.','令 $MR_1=MR_2=MC$，並檢查利潤 Hessian 矩陣為負定。')],
...
function three(rng, level) {
  ...
  return {id:'opt-multi/three-var',level,vars:[],domain:{},
    prompt:both(`At a stationary point, $H=\\begin{pmatrix}${s*a}&${h}&${j}\\\\${h}&${s*b}&${k}\\\\${j}&${k}&${s*c}\\end{pmatrix}$. Find its three leading principal minors and classify the point.`, `在平穩點處，$H=\\begin{pmatrix}${s*a}&${h}&${j}\\\\${h}&${s*b}&${k}\\\\${j}&${k}&${s*c}\\end{pmatrix}$。求三個領導主要子行列式並判別。`),
    ...
    misconceptions:[error('d2',D2+1,'Subtract the squared off-diagonal entry in the second determinant.','第二領導主要子行列式須減去非對角元素的平方。')],
    ...
    solution:[both(`The minors are $D_1=${D1}$, $D_2=${D2}$, $D_3=${D3}$.`, `領導主要子行列式為 $D_1=${D1}$、$D_2=${D2}$、$D_3=${D3}$。`),both(`Their signs are ${kind==='max'?'negative, positive, negative':'positive, positive, positive'}. Therefore the Hessian is ${kind==='max'?'negative':'positive'} definite and the stationary point is a strict local ${kind==='max'?'maximum':'minimum'}.`, `符號依序為${kind==='max'?'負、正、負':'正、正、正'}，故 Hessian 矩陣${kind==='max'?'負':'正'}定，平穩點是嚴格局部${kind==='max'?'極大':'極小'}值。`)]};
}
export const optMultiGenerators=[
  {id:'foc-2var',title:both('Two-variable stationary point','雙變數平穩點'),levels:[1,2],minDistinct: 40, generate:foc},
  {id:'hessian',title:both('Hessian entries and determinant','Hessian 矩陣元素與行列式'),levels:[2],minDistinct: 40, generate:hessian},
  {id:'classify-2var',title:both('Classify a stationary point','判別平穩點'),levels:[2],minDistinct: 40, generate:classify},
  ...
  {id:'three-var',title:both('Three-variable Hessian','三變數 Hessian 矩陣'),levels:[3],minDistinct: 40, generate:three}
];
